                    ...
                    try
                    {
                        ...
                        foreach (VisualNovel visualNovel in visualNovels)
                        {
                            ...
                            if (visualNovel.Tags.Length > 0)
                            {
                                ...
                                {
                                    sql =
                                        "INSERT OR REPLACE INTO VnInfoTags VALUES(@PK_Id, @VnId, @TagId, @TagName, @Score, @Spoiler);";
                                    cmd = new SQLiteCommand(sql, connection, transaction);
                                    cmd.Parameters.AddWithValue("@PK_Id", null);
                                    ...
                                    cmd.Parameters.AddWithValue("@Spoiler",
                                        CheckForDbNull(tag.SpoilerLevel.ToString() ?? null));
                                    cmd.ExecuteNonQuery();
                        ...
                        foreach (var character in characters)
                        {
                            ...
                            {
                                sql ="INSERT OR REPLACE INTO VnCharacterTraits VALUES(@PK_Id, @CharacterId, @SpoilerLevel)";
                                cmd = new SQLiteCommand(sql, connection, transaction);
                                cmd.Parameters.AddWithValue("@PK_Id", null);
                                cmd.Parameters.AddWithValue("@CharacterId", CheckForDbNull(trait.Id));
                                cmd.Parameters.AddWithValue("@SpoilerLevel",
                                    CheckForDbNull(trait.SpoilerLevel.ToString() ?? null));
                                cmd.ExecuteNonQuery();
                            }
                            ...
                            foreach (VndbSharp.Models.Character.VisualNovelMetadata vn in character.VisualNovels)
                            {
                                ...
                                cmd.Parameters.AddWithValue("@SpoilerLevel",
                                    CheckForDbNull(vn.SpoilerLevel.ToString() ?? null));
